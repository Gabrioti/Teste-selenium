...
def principal(lista_cnpjs, tipos_cnd, solicitar_captcha=None, interface=None):
    ...
    for cnpj_idx, cnpj in enumerate(lista_cnpjs):
        ...
        # 2. Abrimos as pistas de corrida
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            # 3. Criamos a lista (array) de TAREFAS com base nas seleções
            tarefas = []
            futuros_rastreados = {}
            
            if "ESTADUAL" in tipos_cnd_norm and not flag_cancelamento:
                tarefas.append(executor.submit(teste_ESTADUAL.recolher_estadual, cnpj, site[0], navegadores_fixos[0] if navegadores_fixos else None, pasta_download))
            
            if "FGTS" in tipos_cnd_norm and not flag_cancelamento:
                idx_fgts = 1 if "ESTADUAL" in tipos_cnd_norm else 0
                tarefas.append(executor.submit(teste_FGTS.recolher_FGTS, cnpj, site[2], navegadores_fixos[idx_fgts] if len(navegadores_fixos) > idx_fgts else None, pasta_download))
            ...
            if "AGEHAB" in tipos_cnd_norm and not flag_cancelamento:
                ...
                # 3. Põe no rastreador com o nome exato da coluna da interface!
                futuros_rastreados[futuro_agehab] = "AGEHAB"

            if "COMPRASNET" in tipos_cnd_norm and not flag_cancelamento:
                idx_comprasnet = 4 if ("ESTADUAL" in tipos_cnd_norm and "FGTS" in tipos_cnd_norm and "AGEHAB" in tipos_cnd_norm and "TRABALHISTA" in tipos_cnd_norm) else (3 if ("ESTADUAL" in tipos_cnd_norm and "FGTS" in tipos_cnd_norm and "AGEHAB" in tipos_cnd_norm) or ("ESTADUAL" in tipos_cnd_norm and "FGTS" in tipos_cnd_norm and "TRABALHISTA" in tipos_cnd_norm) or ("ESTADUAL" in tipos_cnd_norm and "AGEHAB" in tipos_cnd_norm and "TRABALHISTA" in tipos_cnd_norm) or ("FGTS" in tipos_cnd_norm and "AGEHAB" in tipos_cnd_norm and "TRABALHISTA" in tipos_cnd_norm) else (2 if ("ESTADUAL" in tipos_cnd_norm and "FGTS" in tipos_cnd_norm) or ("ESTADUAL" in tipos_cnd_norm and "AGEHAB" in tipos_cnd_norm) or ("FGTS" in tipos_cnd_norm and "AGEHAB" in tipos_cnd_norm) or ("ESTADUAL" in tipos_cnd_norm and "TRABALHISTA" in tipos_cnd_norm) or ("FGTS" in tipos_cnd_norm and "TRABALHISTA" in tipos_cnd_norm) or ("AGEHAB" in tipos_cnd_norm and "TRABALHISTA" in tipos_cnd_norm) else (1 if "ESTADUAL" in tipos_cnd_norm or "FGTS" in tipos_cnd_norm or "AGEHAB" in tipos_cnd_norm or "TRABALHISTA" in tipos_cnd_norm else 0)))
                futuro_comprasnet = executor.submit(teste_COMPRASNET.recolher, cnpj, site[5], nav, pasta_download, solicitar_captcha)
                
                # 2. Põe na lista de espera
                tarefas.append(futuro_comprasnet)
                
                # 3. NOVO: Cadastra no rastreador exatamente com o nome que está na coluna do painel!
                futuros_rastreados[futuro_comprasnet] = "ComprasNet"
            print("Sites abertos!")

            # 4. Roteador Municipal: Adiciona as tarefas extras na lista se precisar
            if "MUNICIPAL" in tipos_cnd_norm:
                for municipio in cidades_da_empresa:
                    ...
                    print(f"[{nome_cidade}] Iniciando verificação do município: {nome_cidade}")
                    ...
                    # SE FOR AUTOMATIZADO: Cria um navegador exclusivo pra ela e já aplica o CDP CMD
                    ...
                    # Verifica qual é a cidade e ADICIONA no array de tarefas (.append)
                    if nome_cidade == "Águas Lindas":
                        # 1. Cria a tarefa e salva numa variável
                        futuro_cidade = executor.submit(Águas_Lindas.recolher, cnpj, site_da_cidade, nav_mun, pasta_download)
                        # 2. Põe na lista pro Python esperar ela terminar
                        tarefas.append(futuro_cidade)
                        # 3. NOVO: Põe no rastreador para o histórico saber quem ela é
                        futuros_rastreados[futuro_cidade] = nome_cidade
                    elif nome_cidade == "Cidade Ocidental":
                        tarefas.append(executor.submit(Cidade_Ocidental.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade == "Valparaiso":
                        tarefas.append(executor.submit(Valparaiso.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade == "Formosa":
                        tarefas.append(executor.submit(Formosa.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade == "Catalão":
                        tarefas.append(executor.submit(Catalão.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade == "Aparecida de Goiânia":
                        tarefas.append(executor.submit(Aparecida_de_Goiânia.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade == "Luziana":
                        tarefas.append(executor.submit(Luziana.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade == "Goianésia":
                        tarefas.append(executor.submit(Goianésia.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade == "Novo Gama":
                        tarefas.append(executor.submit(Novo_Gama.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade == "Aragoiania":
                        tarefas.append(executor.submit(Aragoiania.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade == "Paraúna":
                        tarefas.append(executor.submit(Paraúna.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade == "Abadiânia":
                        tarefas.append(executor.submit(Abadiânia.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade == "Santo Antônio":
                        tarefas.append(executor.submit(Santo_Antônio.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade == "Mara Rosa":
                        tarefas.append(executor.submit(Mara_Rosa.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade == "Terezópolis":
                        tarefas.append(executor.submit(Terezópolis.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade == "Nerópolis":
                        tarefas.append(executor.submit(Nerópolis.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade == "Flores":
                        tarefas.append(executor.submit(Flores.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade == "Iporá":
                        tarefas.append(executor.submit(Iporá.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade == "Porangatu":
                        tarefas.append(executor.submit(Porangatu.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade == "Bom Jesus":
                        tarefas.append(executor.submit(Bom_Jesus.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade in ("Anápolis", "Ánapolis"):
                        tarefas.append(executor.submit(Anápolis.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade == "Goiânia":
                        tarefas.append(executor.submit(Goiânia.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade == "Valparaiso":
                        tarefas.append(executor.submit(Valparaiso.recolher, cnpj, site_da_cidade, nav_mun, pasta_download, solicitar_captcha=solicitar_captcha))
                    elif nome_cidade == "Caldas Novas":
                        tarefas.append(executor.submit(Caldas_Novas.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade == "Campo Alegre":
                        tarefas.append(executor.submit(Campo_Alegre.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade == "Nova Veneza":
                        tarefas.append(executor.submit(Nova_Veneza.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade == "Senador Canedo":
                        tarefas.append(executor.submit(Senador_Canedo.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade == "Planaltina":
                        tarefas.append(executor.submit(Planaltina.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    elif nome_cidade == "Itaberai":
                        tarefas.append(executor.submit(Itaberai.recolher, cnpj, site_da_cidade, nav_mun, pasta_download))
                    else:
            ...
            if cidades_da_empresa == None:
                print("Lista vazia")
            
            # 5. O código pausa AQUI e espera TODAS as tarefas da lista terminarem (Fixas + Municipais)
            concurrent.futures.wait(tarefas)

            # =========================================================
            # NOVO: VERIFICA OS RESULTADOS PARA SALVAR NO HISTÓRICO
            # =========================================================
            houve_falha = False
            
            for futuro_concluido, nome_da_certidao in futuros_rastreados.items():
                try:
                    # Puxa o resultado que a prefeitura retornou (Ex: False, "Bloqueio: Débitos")
                    resultado = futuro_concluido.result()
                    
                    # Verifica se a cidade devolveu a nossa dupla
                    ...
                    print(f"Erro ao capturar resultado da tarefa {nome_da_certidao}: {e}")
                    
            # Se alguma prefeitura deu erro, atualizamos a tela em tempo real para pintar de vermelho!
            if houve_falha:
                # O if interface existe porque essa função pode rodar via terminal também
                try:
        ...
        if "TRABALHISTA" in tipos_cnd_norm and not flag_cancelamento:
            teste_TRABALISTA.recolher(cnpj, site[5], nav_trabalhista, pasta_download, solicitar_captcha, False)

            futuro_trabalhista = executor.submit(teste_TRABALISTA.recolher, cnpj, site[4], nav, pasta_download, solicitar_captcha)
            
            # 2. Põe na lista de espera
            tarefas.append(futuro_trabalhista)
            
            # 3. NOVO: Cadastra no rastreador para pintar de vermelho se falhar
            futuros_rastreados[futuro_trabalhista] = "Trabalhista"
